	...
	//L'utilisateur supprime certainement son avatar sur l'appli			   
				   ...
				   $userBD->setAvatar(null);
					...
					$em->remove($holdAvatar);
	 				...
	 				$userfind['id'] = $userBD->getId();
					$userfind['email'] =  $userBD->getEmail();
					$userfind['password'] = $userBD->getPassword();
					$userfind['firstname'] = $userBD->getPrenom();
					$userfind['lastname'] = $userBD->getNom();
					$userfind['username'] = $userBD->getusername();
					$userfind['birthday'] = $userBD->getDateNaissance();
					$userfind['kind'] = $userBD->getGenre();
					$userfind['phone'] = $userBD->getTelephone();
					$userfind['type'] = $userBD->getTypeUtilisateur();
					$userfind['monnaie'] = $userBD->getMonnaie();
					//$userfind['photo'] = $user->getPhoto();
					$userfind['photo'] = ($userBD->getAvatar() != null) ? $userBD->getAvatar()->getUrl() : null;
					
						//traitement des langues
						$languesUser = array();
						foreach($userBD->getLangues() as $langue)
						{
						$row['id'] = $langue->getId();
						$row['nom'] =  $langue->getNom();			
						$languesUser[]=$row;	
      ...
      $listeActivites = $em->getRepository('BusinessModelBundle:Activite')->findBy(array(), array('nbVues' => 'DESC'), 4, 0);
        ...
        ->setBody(
            $this->renderView(
                ...
                array('user' => $userId->getNomComplet(),'activity' => $activiteId->getLibelle(),
			        'phone' => $userId->getTelephone(),'email' => $userId->getEmail(), 'logo' => MonMailer::pathLogo())
				 ),
				 null
				 );
      ...
      $userByEmail->setSalt($salt);
		...
		$em->flush();
		 ...
		 $retour= MonMailer::envoyerMail(
                ...
                array('user' => $userByEmail->getNomComplet(), 'password' => $password, 'logo' => MonMailer::pathLogo())
				 ),
				 null
				 );
				 if(!$retour)
